...
def main():
    ...
    config = KERNEL_CONFIGS[kernel_type].copy()
    # Extracted kernels carry the exact model shape and benchmark formulas.
    custom_sizes = getattr(kernel_module, "TEST_SIZES", None)
    if isinstance(custom_sizes, (list, tuple)) and custom_sizes:
        config["test_sizes"] = list(custom_sizes)
    custom_flops = getattr(kernel_module, "FLOPS_FN", None)
    custom_bytes = getattr(kernel_module, "BYTES_FN", None)
    if callable(custom_flops):
        config["flops_fn"] = custom_flops
    if callable(custom_bytes):
        config["bytes_fn"] = custom_bytes
    custom_tolerances = getattr(kernel_module, "TOLERANCES", None)
    if isinstance(custom_tolerances, dict):
        dtype_names = {"float16": torch.float16, "bfloat16": torch.bfloat16, "float32": torch.float32}
        config["tolerances"] = {dtype_names.get(str(dtype), dtype): value for dtype, value in custom_tolerances.items()}
    ...
    try:
        ...
        if args.quick:
            sizes_filter = "model_primary" if getattr(kernel_module, "TEST_SIZES", None) else "large"
