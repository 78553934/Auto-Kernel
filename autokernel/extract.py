...
def parse_shape_info(shape_info_str: str, op_type: str) -> Optional[Dict[str, int]]:
    ...
    # Match key=value pairs first (the profiler and hand-written reports use
    # this form).
    pairs = re.findall(r"([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(\d+)", shape_info_str)
    if not pairs:
        # torch.profiler serializes input shapes as a Python literal, e.g.
        # ``[[1024, 4096], [4096, 4096]]``.  Parse that representation rather
        # than silently replacing a real model shape with a default.
        try:
            parsed = ast.literal_eval(shape_info_str)
        except (SyntaxError, ValueError):
            return None
        lists = [x for x in parsed if isinstance(x, (list, tuple))] if isinstance(parsed, (list, tuple)) else []
        dims = [[int(x) for x in x if isinstance(x, (int, float))] for x in lists]
        if op_type == "matmul" and len(dims) >= 2 and len(dims[0]) >= 2 and len(dims[1]) >= 2:
            return {"M": dims[0][-2], "K": dims[0][-1], "N": dims[1][-1]}
        if op_type in {"layernorm", "rmsnorm"} and dims and len(dims[0]) >= 2:
            return {"batch": max(1, int(__import__("math").prod(dims[0][:-1]))), "dim": dims[0][-1]}
        if op_type == "softmax" and dims and len(dims[0]) >= 2:
            return {"rows": max(1, int(__import__("math").prod(dims[0][:-1]))), "cols": dims[0][-1]}
        return None
...
def extract_kernels(
    ...
    # Initialize the optional SOLAR theory state automatically.  Extraction
    # remains successful if the bridge or its optional dependencies are absent.
    bridge_candidates = [
        os.path.join(SCRIPT_DIR, "integration", "solar_bridge.py"),
        os.path.join(os.path.dirname(SCRIPT_DIR), "integration", "solar_bridge.py"),
    ]
    bridge_path = next((path for path in bridge_candidates if os.path.isfile(path)), bridge_candidates[0])
    theory_state_path = os.path.join(WORKSPACE_DIR, "theory_state.json")
    if os.path.isfile(bridge_path):
        ...
        try:
            result = subprocess.run(bridge_cmd, check=False, capture_output=True, text=True)
            if result.returncode == 0:
                print(f"SOLAR theory state saved to {theory_state_path}")
            else:
                print(f"WARNING: SOLAR theory initialization failed: {result.stderr.strip()}")
        except OSError as exc:
            print(f"WARNING: Could not start SOLAR bridge: {exc}")
