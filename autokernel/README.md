...
# Optional: install the copied SOLAR package for native graph/performance analysis
cd ../solar && pip install -e . && cd ../autokernel
...
# Optional: initialize SOLAR theory-guided search (when integration/ is enabled)
...
The checkout also includes an optional `integration/` bridge to
[NVIDIA SOLAR](https://github.com/NVlabs/SOLAR). It supplies Roofline/theory
headroom to `orchestrate.py`; actual KEEP/REVERT decisions still use the fixed
`bench.py` harness.
