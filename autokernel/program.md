...
## SOLAR Theory-Guided Search

This checkout contains an `integration/` directory that connects AutoKernel to
NVIDIA's SOLAR analysis toolkit. Use it as a theory advisor before and during
the real benchmark loop. SOLAR/Roofline latency is a lower-bound estimate,
never a substitute for `bench.py`.

After profiling, initialize the theory state:
...
```bash
python integration/theory_advisor.py suggest \
  --state workspace/theory_state.json \
  --kernel <kernel_id>
```

Follow the returned strategy. After `bench.py`, update the advisor with the
measured latency:
...
If the advisor returns `SWITCH_STRATEGY`, stop repeating the current family of
changes. If it returns `MOVE_ON`, save the kernel and ask `orchestrate.py` for
the next one. Correctness and KEEP/REVERT decisions always come from
`bench.py`.
...
The extraction step also initializes the bundled SOLAR theory state:
...
```bash
uv run orchestrate.py record kernel_<type>_<rank>.py <tflops> keep "baseline" --run-log run.log
```
...
```bash
uv run orchestrate.py record <file> <tflops> keep|revert "<description>" --run-log run.log
```
