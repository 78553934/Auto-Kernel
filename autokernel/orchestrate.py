...
Usage:
    ...
    uv run orchestrate.py record <kernel_file> <throughput_tflops> <status> <description> [--latency-us N] [--run-log run.log]
...
Writes: workspace/orchestration_state.json, workspace/aggregate_report.md,
        workspace/theory_state.json (when the SOLAR bridge is enabled)
"""
...
THEORY_STATE_PATH = WORKSPACE / "theory_state.json"

# The bridge is bundled under autokernel/integration. AutoKernel remains
# runnable without it; the extra scheduling signals are enabled automatically
# when the package is present.
THEORY_ADVISOR = None
try:
    sys.path.insert(0, str(SCRIPT_DIR.parent))
    from integration.theory_advisor import (  # type: ignore
        load_state as load_theory_state,
        save_state as save_theory_state,
        update_item as update_theory_item,
        _parse_metrics as parse_theory_metrics,
    )
    THEORY_ADVISOR = True
except Exception:
    load_theory_state = None
    save_theory_state = None
    update_theory_item = None
    parse_theory_metrics = None
...
def _default_kernel_entry(
    ...
    return {
        ...
        "theory_kernel_id": f"kernel_{op_type}_{rank}",
        "theory_action": None,
        "theory_strategy": None,
        "theory_gap_ratio": None,
    }
...
def _should_move_on(kernel: dict) -> tuple[bool, str]:
    ...
    """
    theory = _theory_suggestion(kernel)
    if theory and theory.get("action") == "MOVE_ON":
        return True, f"SOLAR theory advisor: {theory.get('reason', 'theory floor reached')}"
...
def _theory_suggestion(kernel: dict) -> dict | None:
    """Read the live SOLAR theory state for a kernel, if available."""
    if not THEORY_ADVISOR or not THEORY_STATE_PATH.exists() or load_theory_state is None:
        return None
    try:
        theory_state = load_theory_state(THEORY_STATE_PATH)
        kernel_id = kernel.get("theory_kernel_id") or f"kernel_{kernel.get('op_type')}_{kernel.get('rank')}"
        item = theory_state.get("kernels", {}).get(kernel_id)
        if item is None:
            return None
        # Import lazily to keep AutoKernel's no-bridge path dependency-free.
        from integration.theory_advisor import _suggest  # type: ignore
        suggestion = _suggest(item)
        kernel["theory_action"] = suggestion.get("action")
        kernel["theory_strategy"] = suggestion.get("strategy")
        kernel["theory_gap_ratio"] = suggestion.get("gap_ratio")
        return suggestion
    except Exception as exc:
        print(f"WARNING: theory advisor unavailable: {exc}")
        return None


def _update_theory_after_record(
    kernel: dict,
    latency_us: float | None = None,
    correctness: str = "PASS",
    keep: bool | None = None,
) -> dict | None:
    """Synchronize a benchmark result into theory_state.json."""
    if not THEORY_ADVISOR or not THEORY_STATE_PATH.exists() or load_theory_state is None or update_theory_item is None:
        return None
    if latency_us is None:
        return _theory_suggestion(kernel)
    try:
        theory_state = load_theory_state(THEORY_STATE_PATH)
        kernel_id = kernel.get("theory_kernel_id") or f"kernel_{kernel.get('op_type')}_{kernel.get('rank')}"
        item = theory_state.get("kernels", {}).get(kernel_id)
        if item is None:
            return None
        suggestion = update_theory_item(
            item,
            {"latency_us": float(latency_us), "correctness": correctness},
            keep=keep,
        )
        save_theory_state(THEORY_STATE_PATH, theory_state)
        kernel["theory_action"] = suggestion.get("action")
        kernel["theory_strategy"] = suggestion.get("strategy")
        kernel["theory_gap_ratio"] = suggestion.get("gap_ratio")
        return suggestion
    except Exception as exc:
        print(f"WARNING: failed to update theory state: {exc}")
        return None
...
def cmd_next(state: dict) -> None:
    ...
    if should_move:
        ...
        # Continue current
        kname = Path(current["file"]).name
        theory = _theory_suggestion(current)
        ...
        if theory:
            gap = theory.get("gap_ratio")
            gap_text = f"{gap:.1%}" if isinstance(gap, (int, float)) else "N/A"
            print(f"  SOLAR: {theory.get('action')} | strategy={theory.get('strategy')} | theory gap={gap_text}")
...
def cmd_record(
    ...
    *status* is one of: kept, revert, failed, crash, timeout
    ...
    # Parse the run log up front so the latency lands in both the TSV row
    # and the SOLAR theory state.
    if latency_us is None and run_log and parse_theory_metrics is not None:
        try:
            parsed = parse_theory_metrics(Path(run_log).read_text(encoding="utf-8"))
            parsed_latency = parsed.get("latency_us")
            if isinstance(parsed_latency, (int, float)):
                latency_us = float(parsed_latency)
        except OSError as exc:
            print(f"WARNING: could not read run log for theory update: {exc}")
    ...
    row = {
        ...
        "latency_us": f"{latency_us:.4f}" if latency_us is not None else "",
        "pct_peak": "",
    ...
    theory_suggestion = _update_theory_after_record(
        target,
    ...
    if theory_suggestion:
        print(
            f"  SOLAR advisor: {theory_suggestion.get('action')} "
            f"({theory_suggestion.get('strategy')})"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rec.add_argument("--latency-us", type=float, help="Measured kernel latency; updates SOLAR theory state")
    rec.add_argument("--run-log", help="bench.py log; latency_us is parsed when --latency-us is omitted")
